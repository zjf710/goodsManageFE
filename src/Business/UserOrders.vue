  ...
  <view-box>
    ...
    <div class="gm-goods-list">
      ...
      <div class="goods-item" v-for="goods in goodsList">
        ...
        <a v-if="userType === '1' && goods.status !== 0" class="phone" :href="`tel:${userType === '1' ? goods.orderTaker : goods.publish_man}`">
        ...
        <x-dialog v-model="showDetail" class="goods-info-dialog" hide-on-blur>
          <div class="goods-info-detail">
            ...
            <flexbox class="btn-container">
              <flexbox-item v-if="userType === 2 && goodsInfo.status === 1">
                <x-button type="primary" @click.native="changeStatus">货单开始</x-button>
              </flexbox-item>
              <flexbox-item v-if="userType === 2 && goodsInfo.status === 2">
                <x-button type="primary" @click.native="changeStatus">已完成</x-button>
              </flexbox-item>
              <flexbox-item v-if="userType === 1 && goodsInfo.status === 3">
                <x-button type="primary" @click.native="changeStatus">确认完成</x-button>
              </flexbox-item>
              <flexbox-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeStatus() {
        const params = {
          status: this.goodsInfo.status + 1,
